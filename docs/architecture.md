...
WeChatSendGuard 提供 Windows 10/11 x64 安装包和 macOS Universal DMG。macOS 使用 Ad-hoc 签名且未公证，首次打开可能需要用户按 Gatekeeper 提示允许；具体微信版本兼容性仍以真实客户端人工清单为准。Windows 与 macOS 不共享二进制、安装器或系统 API，Linux 仍未实现。
...
`guard-core` 不调用 Win32、Slint 或微信 API，也不保存某个 Windows 安装目录作为默认值。公共设置中允许存在可选的 `trustedWeixinExecutablePath` 字段，但它的含义、自动识别流程与校验均由 Windows 适配器拥有。`desktop-ui` 只能显示状态和发出意图，不能读取外部应用、安装键盘钩子或注入按键。
...
托盘暂停和临时放行都是运行时临时状态，沿用 1、5、15 分钟选项，不写入设置文件。暂停开始时取消待确认请求并停止输入观察；状态轮询在到期后恢复原有观察配置。临时放行只在当前服务实例内生效，到期后恢复原规则；应用重启后两类临时状态都从初始状态开始。手动关闭守护也会清除剩余的临时暂停，之后是否启用由持久化的总开关决定。

## Windows 信任与兼容性边界

Windows 在启动时和运行期间自动扫描正在运行的微信窗口。检测到唯一的有效 `Weixin.exe` 路径后，应用会记录其实际绝对路径并用于精确身份校验；未检测到微信或同时检测到多个安装位置时，不更新已有识别结果，也不会仅依据进程名信任目标。应用不要求用户填写固定安装路径。

路径比较仅忽略 Windows 路径的大小写和分隔符差异，不会因为同名进程而信任目标。

保存自动识别结果时，`desktop-app` 会先持久化设置，再让 `WindowsContextProvider` 使旧前台快照失效并重新观察。路径无效、进程访问失败、权限级别不一致、UI Automation 控件缺失、聊天身份或编辑框焦点无法确认时，前台上下文均视为不可保护，绝不注入按键。
...
Windows 设置文件位于 `%LocalAppData%\WeChatSendGuard\settings.json`，macOS 设置文件位于 `~/Library/Application Support/WeChatSendGuard/settings.json`；两者使用 `camelCase` 字段和 PascalCase 枚举值。应用版本和配置架构版本独立维护。新增的可选路径字段是向后兼容的扩展，旧设置文件不包含它时仍等待 Windows 自动识别正在运行的微信；macOS 忽略该 Windows 专用字段。

未识别到微信时不会写入路径字段；成功识别的绝对路径会以如下形式保存：
...
系统窗口句柄、托盘菜单对象、文件选择器和启动项属于平台适配器。界面只通过回调表达“保存设置”“打开设置”等意图；组合根负责将这些意图交给当前平台服务。
