...
当前发布版本为 `1.3.1`，发布产物包含 Windows 10/11 x64 安装包和 macOS 11+ Universal DMG。macOS 包使用免费 Ad-hoc 签名，未使用 Apple Developer ID 或公证，首次打开可能需要按系统提示允许。项目采用 Rust + Slint 原生实现，终端用户不需要安装 .NET、Java、浏览器运行时或其他额外运行时。
...
- “关于”页默认会在启动后自动检查正式版更新，也可手动检查、忽略当前版本的自动提醒，或下载并安装；更新检查只访问 GitHub Releases，应用内更新会自动校验 SHA-256；
...
普通安装不要求手动执行 SHA-256 校验；应用内更新会自动下载并校验对应资产。若系统显示“应用已损坏”，请先从 GitHub Release 重新下载；仍无法打开时，可在终端执行以下高级排障命令：
