...
当前正式版本为 `1.1.0`，只发布 Windows 10/11 x64 安装包。项目已采用 Rust + Slint 原生实现，终端用户不需要安装 .NET、Java、浏览器运行时或其他额外运行时。
...
Windows 是 `1.1.0` 的唯一交付平台，但以下内容是后续 macOS、Linux 适配必须保持的产品契约：
